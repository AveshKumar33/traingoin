...
const CustomizeDotProductCard = ({
  ...
  useEffect(() => {
    if (
      cartProduct &&
      cartProduct?.productCombination &&
      cartProduct?.productCombination?.length > 0
    ) {
      const productPrice = [];

      for (let combination of cartProduct?.productCombination) {
        const product = combination?.productId;
        // for front
        ...
        productPrice.push({
          ...
          price: CustomizedFinialAmount(FrontPrice + BackPrice, product),
        });
      }
      ...
      setPrice(total * cartProduct?.quantity);
      setInitialPrice(total);
      setQuantity(cartProduct?.quantity);
    }
  }, [cartProduct, FinialAmount, calculateCustomizedPrice]);
